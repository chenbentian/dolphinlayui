...
import com.bt.dolphin.common.util.EncryptUtil;
...
public class ShiroUtil {
     ...
     * 获取当前用户对象
     */
    public static SysUserVo getSubject() {
   ...
   /*     if (user != null && !Hibernate.isInitialized(user.getDept())) {
            try {
                ...
                SysUserVo reload = userService.getById(user.getId());
     ...
     * 获取当前用户角色列表
     */
    public static Set<SysRoleVo> getSubjectRoles() {
        SysUserVo user = (SysUserVo) SecurityUtils.getSubject().getPrincipal();
