...
layui.use(['element', 'form', 'layer', 'upload','layedit'], function () {
    ...
    /* 新建或切换标签栏 */
    tabs = function (url,menuId) {
        var item = $('[lay-menuId="' + menuId + '"]');
        var bootLay = $('[lay-id="' + menuId + '"]');
        if (bootLay.length === 0) {
       ...
       element.tabChange('iframe-tabs', menuId);
       $(".layui-side .layui-this").removeClass("layui-this");
       $('[lay-menuId="' + menuId + '"]').parent().addClass("layui-this");
       $(".layui-tab-title .layui-this").removeClass("layui-this");
       $('[lay-id="' + menuId + '"]').addClass("layui-this");
    };

    /* 监听导航栏事件，实现标签页的切换 */
    element.on("nav(layui-nav-side)", function ($this) {
        var url = $this.attr('lay-url');
        var menuId = $this.attr('lay-menuId');
        var menuKind = $this.attr('lay-menuKind');
        if(menuKind == "MenuItem"){
        	tabs(url,menuId);	
        }
    });
    ...
    window.onhashchange = function (e) {
        var menuId = location.hash.replace(/^#/, '');
        tabs("",menuId);
    };
    //默认打开首页
    window.openTab = function(url,menuId){
        var index = $(".layui-layout-admin .layui-side .layui-nav-item")[0];
		  $(index).children("a").attr("lay-icon", "true");
		  url = $(index).children("a").attr("lay-url");
